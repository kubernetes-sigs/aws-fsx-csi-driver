## Dynamic Provisioning with DeploymentType
This example shows how to create a FSx for Lustre filesystem using persistence volume claim (PVC) with DeploymentType specified
...
parameters:
  subnetId: subnet-0cd1914f0b9041ebe
  securityGroupIds: sg-07662e27978b3b360,sg-057948b58dd38e68b
  s3ImportPath: s3://evanzempty
  s3ExportPath: s3://evanzempty
...
* deploymentType: FSx for lustre now provides three deployment types which uses different EC2 instances types
...
spec:
  ...
  resources:
    requests:
      storage: 1200Gi
```
Update `spec.resource.requests.storage` with the storage capacity to request. For deployment SCRATCH_2, The storage capacity value will be rounded up to 1200 GiB, 2400 GiB, or a multiple of 2400 GiB.

### Deploy the Application
Create PVC, storageclass and the pod that consumes the PV:
```sh
>> kubectl apply -f examples/kubernetes/dynamic_provisioning_withdeploymenttype/specs/storageclass.yaml
>> kubectl apply -f examples/kubernetes/dynamic_provisioning_withdeploymenttype/specs/claim.yaml
>> kubectl apply -f examples/kubernetes/dynamic_provisioning_withdeploymenttype/specs/pod.yaml
```
